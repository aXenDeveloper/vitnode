...
// string through. A plugin is compiled into its own `dist` and imported by
// whichever app installed it, so a type-level dependency on one host's framework
// is one every installing app inherits - keep it framework-neutral. `use-intl`
// must be a direct dependency of this package for the reference above to resolve
