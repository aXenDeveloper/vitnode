...
/**
 * `/admin/core/staff/admins/create` - adding a role or a user to the admins
 * group.
 *
 * The permission (`staff_admins.can_create`), the two pickers, the write and
 ...
 */
export const Route = createFileRoute('/_admin/admin/core/staff/admins/create')({
  // `head` after `loader`, always - see the note in `_main/discover.tsx`.
  loader: async ({ context }) =>
    await loadAdminStaffCreateRoute({ ...context, type: 'admin' }),
  head: ({ loaderData }) => pageHead({ ...loaderData }),
  component: AdminStaffCreateRoute,
  staticData: {
    breadcrumb: <AdminStaffCreateBreadcrumbContent type="admin" />,
  },
})
