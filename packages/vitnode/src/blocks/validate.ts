...
/**
 * Whole pixels, or one of the names areas were laid out with before them.
 *
 * A stored name is read and written back as its number, so a layout is
 * normalised the first time somebody saves the page it is on and nothing has
 * to migrate the table.
 */
const zodAreaSpacing = z
  .union([
    z.int().min(AREA_SPACING_MIN).max(AREA_SPACING_MAX),
    z.enum(AREA_LEGACY_SPACINGS).transform(named => AREA_LEGACY_SPACING[named]),
  ])
  .optional();
...
const zodAreaLayout = z.strictObject({
  ...
  marginX: zodAreaSpacing,
  marginY: zodAreaSpacing,
});
