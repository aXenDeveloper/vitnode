 ...
 * lives here rather than once per host: an application hands the two generated
 * values to `pluginRouteSpecs` and the result to `withPluginRoutes`, and that is
 * the whole of its plugin-routing code.
 *
 ...
 * owns the public one, so there is nothing to prefix.
 */
