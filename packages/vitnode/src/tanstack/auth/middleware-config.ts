...
export interface MiddlewareConfigState extends MiddlewareConfig {
   ...
   * fallback below.
   *
   * Not part of the API schema - it is this layer's record of whether the read
 ...
 * to the application would cost two round trips instead of one - the same
 * reasoning the public feed reads follow.
 *
 ...
 * the `server-only` marker in it - never reaches the browser. It is also the one
 * Start primitive a package may declare: uncompiled, on a server, its runtime
 * stub prefers the `.server()` branch, which is exactly what a server wants.
 */
