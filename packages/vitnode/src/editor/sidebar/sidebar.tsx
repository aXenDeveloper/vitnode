...
const PANEL_MOTION_CLASS =
  "min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain animate-in fade-in duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] motion-reduce:animate-none";

const PANEL_FORWARD_CLASS = `${PANEL_MOTION_CLASS} slide-in-from-right-4 rtl:slide-in-from-left-4`;

const PANEL_BACK_CLASS = `${PANEL_MOTION_CLASS} slide-in-from-left-4 rtl:slide-in-from-right-4`;
...
const EditorSidebarPanels = ({
  ...
  return (
    <aside
      aria-label={t("title")}
      className={cn(
        "border-border bg-card text-card-foreground fixed start-0 end-0 bottom-0 z-40 flex max-h-(--editor-sheet-height) flex-col overflow-hidden rounded-t-2xl border-t shadow-lg transition-transform duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:transition-none md:start-auto md:top-0 md:max-h-none md:w-(--editor-sidebar-width) md:rounded-none md:border-s md:border-t-0 md:shadow-none",
