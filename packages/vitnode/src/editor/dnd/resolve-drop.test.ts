...
describe("an area dragged out of the Layout section", () => {
  const fromLayout = (): EditorDragSource => ({ kind: "catalog-area" });
  ...
  it("is read back off the drag payload", () => {
    expect(readDragSource({ kind: "catalog-area" })).toEqual({
      kind: "catalog-area",
    });
  });

  it("is never itself a drop target", () => {
    expect(readDropTarget({ kind: "catalog-area" }, "before")).toBeNull();
  });

  it("lands where the pointer says, by the edge it was dropped on", () => {
    expect(
      resolveDrop({
        capabilities: accepting("*"),
        source: fromLayout(),
    ...
    ).toEqual({ kind: "insert-area", to: into("page:main"), toIndex: 1 });
    ...
    ).toEqual({ kind: "insert-area", to: into("page:main"), toIndex: 2 });
  });

  it("lands last when it was dropped on the zone rather than a node", () => {
    expect(
      resolveDrop({
        capabilities: accepting("*"),
        source: fromLayout(),
  ...
  it("is refused by an area, because an area cannot hold another", () => {
    expect(
      dropRejection({
        capabilities: accepting("*"),
        source: fromLayout(),
  ...
  it("is refused at a zone root that is already at the node cap", () => {
    const decision = decideDrop({
      capabilities: accepting(undefined),
      capacity: { source: null, target: holds(CONTENT_BLOCKS_ABSOLUTE_MAX) },
      source: fromLayout(),
      target: onContainer("page:main"),
      targetNodeCount: CONTENT_BLOCKS_ABSOLUTE_MAX,
    });
  ...
  it("still lands in a zone that is at its block max, because it carries none", () => {
    const decision = decideDrop({
      capabilities: accepting(undefined),
      capacity: { source: null, target: holds(2, { blocks: 4, max: 4 }) },
      source: fromLayout(),
      target: onContainer("page:main"),
      targetNodeCount: 2,
    });
    ...
    expect(decision.rejection).toBeNull();
    expect(decision.resolved?.kind).toBe("insert-area");
  });

  it("asks no allowlist, because an empty area carries no block to check", () => {
    expect(
      dropRejection({
        capabilities: accepting(["core:text"], ["core:text"]),
        source: fromLayout(),
        target: onContainer("page:main"),
      }),
    ).toBeNull();
  ...
  it("is not drawn into an area by collision precedence", () => {
    const collisions = [
      { id: areaDroppableId(into("page:main", "area-1")) },
      { id: nodeDraggableId(areaNodeRef("page:main", "area-1")) },
    ];

    expect(preferInnerCollisions(collisions, fromLayout())).toEqual([
      { id: nodeDraggableId(areaNodeRef("page:main", "area-1")) },
    ]);
  });
});
