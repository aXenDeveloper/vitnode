...
import type { ContentEditorShellMode } from "../blocks/editor-shell";
...
export interface EditorRootProps {
  ...
  /** Edit mode has ended and the sidebar is sliding out: the page is already public. */
  closing: boolean;
  onExit?: () => void;
...
const EditorShell = ({
   ...
   * itself, rather than by the seam that lazily loads it. On a first open the
   * chunk and the sidebar's messages take a moment to arrive, and a shell that
   * started moving when edit mode was merely *requested* would slide the page
   * over to a sidebar that is not there yet. Asking from inside the editor means
   * the padding and the panel begin on the same frame, first time and every time.
   */
  const setShell = useEditWidgets()?.setShell;
  const shell: ContentEditorShellMode | null = closing
    ? null
    : preview
      ? "preview"
      : "editing";
  ...
  return (
    <VisualEditorContext value={editor}>
      <EditorDndProvider>
        ...
        <EditorSidebar closing={closing} preview={preview} />
      </EditorDndProvider>

      {closing ? null : <UnsavedChangesGuard />}
      <LeaveConfirmDialog
        onConfirm={() => {
          setLeaving(false);
          onExit?.();
