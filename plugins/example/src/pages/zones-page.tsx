...
const EDIT_MODE_CHECKS = [
  "Edit widgets lives in the user menu in the site header, not on this page, so it is one click away wherever you are. It is shown only to a member holding the moderator permission example.widgets/can_edit - or to a root role - and only while an editable page is on screen. Sign out, or walk to a page that declares no zones, and the entry is gone.",
  ...
  "One sidebar opens on the right and the whole site is padded, never covered: the header, this page and the footer all move left together, exactly as the AdminCP dashboard does when its widget panel opens. Narrow the window below md and the same sidebar becomes a bottom sheet, with the padding moving underneath the site so the footer still scrolls clear of it.",
  "before-footer becomes visible only in edit mode, as a large dashed placeholder with its own Add widget button. Finish editing and it disappears again.",
  ...
  "Add widget on a zone targets it: the sidebar switches to Available Widgets, its header reads For: that zone id, and the zone stays outlined until you clear the target.",
  ...
  "Available Widgets opens with a Layout section above the plugin groups, holding a single Area entry. Click it and an empty area lands in the targeted zone; drag it and it lands exactly where the insertion line says, the same way a block does. Drag it over an area and the drop is refused, because an area cannot hold another one.",
  "Target a zone from inside an area (Add widget on the area) and the header reads For: an area in that zone - and the Layout section disappears, because an area cannot hold another area.",
  ...
  "Select a block and the same sidebar switches to Properties. Back to Available Widgets clears the selection and returns to the catalog.",
  ...
  "Nothing on the page remounts when edit mode turns on: the local counter and the display name you typed both survive Edit widgets and Finish editing, even though the action that started it lives up in the header.",
];
    ...
    >
      <main className="container mx-auto flex max-w-3xl flex-col gap-6 p-4">
        <header className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground leading-relaxed text-pretty">
            An ordinary application page that happens to be editable. Four zones
            sit around a locked profile form, and their blocks come from the
            route loader, so the zones themselves make no request.
          </p>
          <p className="text-muted-foreground text-sm leading-relaxed text-pretty">
