...
than re-reading a record that may have moved on. Delivery is at-least-once: the
search write and the cache expiry are idempotent, but a listener can see one
`published` twice.
