...
<TypeTable
  type={{
    getTranslator: {
      description:
        "next-intl translator for the resolved locale. Pass a locale to override the resolution.",
    ...
    resolveLocale: {
      description: "Runs the resolution above without loading anything.",
...
Emails resolve their own locale - the recipient's, not the sender's - so you rarely call the translator yourself:
