...
<Callout type="info" title="The publish buttons are not here yet">
  Publishing currently happens through the API - `POST /{id}/publish` - or
  through `service.publish` from your own code. The row action, its confirmation
  dialog and the `can_publish` gating land with the AdminCP publication UX.
</Callout>
