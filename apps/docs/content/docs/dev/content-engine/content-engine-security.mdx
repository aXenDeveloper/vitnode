...
- read the record, every language and every language's history;
- create and edit a translation in any enabled locale.

It cannot:

- edit a shared field - `PUT /{id}` is `can_edit`, and the AdminCP's composite
  `PUT /{id}/localized` re-checks `can_edit` in the handler the moment its payload
  carries one, so reaching the form grants nothing;
