...
So a date cursor carries the column's own `::text` and the predicate binds it
...
| date | a canonical Postgres timestamp, `timestamptz`, date or ISO string |
...
that would leave the route answering `500`. Every mismatch is a `400`, and no
malformed value ever reaches Postgres as a failed cast.
