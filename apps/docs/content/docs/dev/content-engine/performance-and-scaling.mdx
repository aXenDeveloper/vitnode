...
The cursor is the primary key, and pages are keyset reads rather than `OFFSET` -
so a page deep into a large collection seeks on the index instead of counting
past every earlier row.

**What is guaranteed:**

- a row already returned never comes back;
- the cursor always advances, so a loop always terminates;
- deleting everything past the cursor ends the walk rather than looping;
- ordering is **total** - the primary key is appended as a tiebreaker, so rows
  sharing an `updatedAt` are not free to move between the two queries a page turn
  issues;
...
<Callout type="warn" title="Order by something that moves with the identifier">
The cursor **is** the identifier. Ordering by a column with no relationship to it
- a title, say - cannot page exactly: rows whose identifiers fall on the wrong
side of the cursor are skipped. Every generated default (`updatedAt`,
`publishedAt`, `createdAt`, `id`) moves with the identifier and is exact; a
custom `orderableFields` entry that does not is a deep-pagination hazard.
</Callout>
