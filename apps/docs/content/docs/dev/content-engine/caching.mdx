...
The bridge itself never throws; the task around it fails when no origin accepted
the request, which is what triggers the retry. Delivery is at-least-once, and
expiring an already-expired tag is a no-op. The reasoning, the auth and the
payload are all in
