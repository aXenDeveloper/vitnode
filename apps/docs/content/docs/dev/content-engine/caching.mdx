...
| Mutation                          | delivery | redirect (old + new) | sitemap |
| --------------------------------- | -------- | -------------------- | ------- |
...
| SEO field edit (still published)   | ✅        | ✅                    | ❌       |

The last row is the one worth reading twice: an edit that only changed what an
already-listed page *says* leaves the sitemap byte-identical, so its tag is not
expired. Everything that adds, removes or moves a line in the file expires it - and on
a localized content type that means each affected locale's file **and** the
locale-less index that enumerates them.
