...
field that did not move. Publishing something already published behaves the
same way: a 200, and no event.

Exactly one event fires per mutation. `published` and `unpublished` never come
with an `updated` alongside them - `status` and `publishedAt` are generated
columns rather than declared fields, so there would be nothing truthful to put
in `changedFields`. Subscribe to all five if you want "anything changed".
