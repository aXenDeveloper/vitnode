...
A [localized](/docs/dev/content-engine/localization) content type is indexed once
per **published translation** rather than once per record, and
`search.pathTemplate` has to carry `{locale}` so each document links to the
language it matched. Everything on this page still applies to each document: the
same allowlist, the same published predicate, the same rebuild contract.

See [Localized search](/docs/dev/content-engine/localized-search).
