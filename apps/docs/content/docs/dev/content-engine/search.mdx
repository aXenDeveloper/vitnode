...
| `titleField` | yes | The result heading. A `text` field, weighted above the body by the index |
...
ranking for every other document in the index. The facet kinds are not text:
...
content type, the record id, the operation and the error, and the newest few show
up in **AdminCP → Advanced → Search** as *Recent sync failures*. There is no
automatic retry.
...
| Automatic retry | Best effort plus a rebuild. A durable outbox is a later addition |
