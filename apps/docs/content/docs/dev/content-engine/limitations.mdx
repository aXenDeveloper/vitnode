...
| Deep relation nesting in a public response | One level, `{ id, label }`. Anything more is a custom route |
...
answering 404. Keep your own redirect table if that matters.
...
they may be running in `apps/api` where there is no Next runtime at all, and
`revalidateTag` needs a request scope a repository does not own.
