...
- **Restricted** — choose exactly which permissions apply, shown as one **tab per
  ...
  administrator entries, the moderator set for moderator entries.
...
The current admin's effective permissions are loaded with the admin session and provided through a context. Gate UI with the `useStaffPermission` hook or the `<StaffPermissionGate>` component from `@vitnode/core/components/staff-permission/provider`:
