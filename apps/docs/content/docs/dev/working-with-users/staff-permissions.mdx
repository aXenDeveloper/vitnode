...
<Callout type="info" title="Visibility only — not a runtime gate">
  `dependsOn` controls what an admin can **see and grant**; it does not change how
  individual checks evaluate. `checkStaffPermission` still tests each permission on
  its own. Keep guarding every action explicitly (see [Backend](#backend)).
</Callout>
