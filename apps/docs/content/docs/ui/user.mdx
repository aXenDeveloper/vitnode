...
<Callout type="info" title="Optional fields">
  A nullable author is `z.number().nullable()`. The field renders the placeholder
  for `null` and never invents a value - clearing one is up to your own control,
  because "no author" and "author not chosen yet" are the same state here.
</Callout>
