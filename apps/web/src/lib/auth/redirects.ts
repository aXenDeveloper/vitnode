 ...
 * through a browser.
 *
 * `#/lib/auth/return-to` is the security half: it decides whether a target is an
 ...
 * in it.
 *
 * `/login` and `/pl/login` are the same route; Stage 3's rewrite strips the
 * prefix before matching and writes it back into every href the router builds.
 * So this constant is deliberately un-prefixed, and nothing in the auth flow
 * concatenates a language onto it.
 */
