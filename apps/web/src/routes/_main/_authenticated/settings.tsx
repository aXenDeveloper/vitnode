...
/**
 * `MigrationLink` rather than the router's `Link`, and it is worth saying why
 * now that it makes no difference: every panel the menu lists is migrated, so
 * every entry resolves to an ordinary client-side navigation. What this keeps is
 * the *rule* - ask the route tree per href, and load the Next.js app for a
 * destination this one does not serve.
 */
function SettingsLayout() {
  return (
    <SettingsLayoutContent LinkComponent={MigrationLink}>
      <Outlet />
    </SettingsLayoutContent>
  )
}
