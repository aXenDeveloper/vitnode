...
The filter is a client component, so `onSearch` is an ordinary function. Read the API directly with `fetcherClient`:
...
<Callout type="info" title="A Server Action works too, with one catch">
  `onSearch` accepts any function returning a promise, so a [Server
  Action](https://react.dev/reference/rsc/server-functions) is a valid answer
  when the lookup genuinely needs a server-only secret. It is the wrong default
  for a read the browser can already make: it is a `POST` back to your app that
  then calls the API - two round trips instead of one - and a Server Action
  cannot be called from an application that is not Next.js.
</Callout>
