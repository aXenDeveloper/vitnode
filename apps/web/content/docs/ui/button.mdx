...
- `secondary` / `outline` / `ghost` - supporting actions, in descending
  loudness.
- `link` - an action that behaves like navigation.
- `success` - confirms or completes something (approve, publish, mark as done).
- `warning` - a risky but reversible action the user should slow down for
  (unpublish, revoke a session).
- `destructive` - it deletes data and it isn't coming back.

`success`, `warning` and `destructive` share the same tinted shape - a soft
coloured surface with matching text - so only the hue tells you how nervous to
be. Colour alone never carries the meaning, so keep the label honest: a green
button still needs to say "Approve".
