...
In a **TanStack Start** app that mounts the API in-process, Vite already serves
`public/` at the site root, so no `serveStatic` is needed - just point the
adapter's public path at a directory inside it:
