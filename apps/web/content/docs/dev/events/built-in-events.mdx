...
simply not announced - and so is one with no categories yet.
...
<TypeTable
  type={{
    ...
    postIds: {
      description:
        "Always empty. The junction to `blog_categories` is ON DELETE RESTRICT, so a category with articles cannot be deleted at all - the field is kept so existing listeners still compile.",
      type: "number[]",
    },
  }}
/>
...
[publication](/docs/dev/content-engine/publication-and-editorial) emits two more:
...
[`editorial`](/docs/dev/content-engine/publication-and-editorial) emits one more, plus two with
[`editorial.scheduling`](/docs/dev/content-engine/publication-and-editorial#step-4-enable-scheduled-publishing):
...
[durable queue task](/docs/dev/content-engine/publication-and-editorial#step-4-enable-scheduled-publishing)
...
[Generated events](/docs/dev/content-engine/services-and-api#step-3-listen-to-typed-content-events).
