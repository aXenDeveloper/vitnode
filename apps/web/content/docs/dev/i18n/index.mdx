...
A package ships whatever languages it ships, so adding one to it is the package author's line to write rather than yours - see [Creating a plugin](/docs/dev/plugins/create). A language nobody has translated still renders: missing keys fall back to `defaultLocale`.
