---
title: Server-side
description: Translate emails and API responses with the request-scoped translator.
---
...
<TypeTable
  type={{
    getTranslator: {
      description:
        "A use-intl translator for the resolved locale. Pass a locale to override the resolution.",
...
`t()` autocompletes from a `global.d.ts` that augments `use-intl`'s `AppConfig`. Import the tree an app actually uses: an API-only app types against the server tree alone, a frontend-only app against the frontend tree, and a single app - one that serves the frontend and runs the API together - against both.
...
<Callout title="`use-intl` has to be a direct dependency" type="info">
  The `/// <reference types="use-intl" />` above only resolves if the package is
  in the app's own `package.json`. Under pnpm's strict `node_modules` layout,
  reaching it through `@vitnode/core` is not enough. `create-vitnode-app` adds it
  for you.
</Callout>
