...
So: no host page wrapper to write, no file in `apps/web` to keep in sync, and no
router to import in the module itself. A page has exactly one home, and the file
you are editing is the file that runs.

See [Route Manifest](/docs/dev/plugins/route-manifest) for every field, and
[Create your first plugin](/docs/dev/tanstack/plugin) for the short version.
