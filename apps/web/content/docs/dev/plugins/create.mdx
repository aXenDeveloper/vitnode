...
## If your app is Next.js

A plugin's routes are read from `routes/manifest.ts` and imported from your
package - nothing is copied, and there is no generated folder to know about.

A **Next.js** host is the exception, and only for the Next.js route files a
plugin may also ship (`src/routes/main`, `src/routes/admin`,
`src/routes/blank`, `src/routes/breadcrumb`). Those are copied into the app, into
`(plugins)` folders. They are generated output: don't edit them, and exclude them
from your editor's search. VS Code gets that from `.vscode/settings.json` out of
the box.

<Callout type="info" title="Two paths, and they never read each other">
  The copier never consults the manifest and the manifest never consults the
  copier, so a plugin can ship both. See [Layouts and
  Pages](/docs/dev/plugins/layouts-and-pages) for the Next.js files and [Route
  Manifest](/docs/dev/plugins/route-manifest) for the host-neutral ones.
</Callout>
