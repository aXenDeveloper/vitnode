...
Use `RoleFormatContent` to render a role's name styled with its `color`. Role
names are translatable, so `name` is the full list of translations and the
component resolves it against the reader's locale from `use-intl`'s context.
...
It reads the locale from context rather than from the request, so it works
anywhere a React tree does - in a table cell, in a dialog, during SSR, in the
browser. Every AdminCP table renders roles through it, and the admin roles list
route returns `name` in exactly this shape.

If you only need the string, `resolveRoleName(role, locale)` from
`@vitnode/core/components/role-name` is the rule the component applies: the
