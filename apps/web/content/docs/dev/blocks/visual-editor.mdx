...
| **Conflict detection**            | Two people editing one page overwrite each other. A save is one transaction, so a row is never half-written, but nothing compares revisions on the way in. |
