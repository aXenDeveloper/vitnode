...
<Callout title="A hidden button has never stopped anybody" type="warn">
  ...
  it must never grant it. See
  [Editable Pages](/docs/dev/blocks/editable-pages#can-i-edit-this).
</Callout>
...
A block that wraps its own rendered text in `<BlockField>` can be typed into from the other end as well - click the heading on the page and edit it there. It is the same `data`, the same reducer and the same Save button, so the two panels never disagree. See [Inline Editing](/docs/dev/blocks/inline-editing).
...
Zone ids run the other way: a zone id is a *place*, so one page may only have one of each. Two `<ContentZone>`s claiming the same id while the editor is open is a [developer error](/docs/dev/blocks/content-zones#one-id-one-place-on-the-page), not a merge.
...
<Callout title="You probably do not write this by hand">
  ...
  definition the server reads. See
  [Editable Pages](/docs/dev/blocks/editable-pages). The rest of this section is
  the contract underneath it, and what to honour if you are writing an adapter
  for somewhere else.
</Callout>
...
| **Permission UI**                 | No roles screen. Who may edit is your page's answer, and the save route's; an [editable page](/docs/dev/blocks/editable-pages#can-i-edit-this) names the moderator permission it wants, but granting it is the ordinary staff screen's job. |
| **Layout administration**         | No AdminCP section listing which pages have been rearranged, no reset-to-default, no export or import, no copying one page's layout onto another. [Deliberately left to a possible V2](/docs/dev/blocks/editable-pages#where-a-layout-is-stored) - the storage shape makes each of them a query on one table, but none is built. |
| **Live collaboration**            | No shared cursors, no presence, no merging two people's edits to **one** zone. Different zones of one page do merge, and a stale write to the same zone is refused rather than applied - see [two moderators, one page](/docs/dev/blocks/editable-pages#two-moderators-one-page) - but nobody watches anybody else arrange anything in real time. |
...
| **Rich text**                     | A `text` or `textarea` field is a plain string, and it stays one. You can type into it [on the page itself](/docs/dev/blocks/inline-editing) or in the properties panel, but no block field holds bold, links or lists. Tiptap ships in this repository and is deliberately nowhere near a block. |
...
`/example/zones` is the manual test page. It has four zones - two around a locked profile form, one narrower sidebar, one empty - blocks from two plugins, a block stored with a variant, an Area holding two blocks, a second Area left empty, and an **Edit widgets** action for anybody holding the page's own moderator permission. It is an [editable page](/docs/dev/blocks/editable-pages), so your changes survive a reload; before anybody saves, it renders the layout the page declares.
