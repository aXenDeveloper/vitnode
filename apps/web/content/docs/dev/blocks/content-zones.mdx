...
Public rendering is therefore **not** a second validation boundary. What the zone does with its allowlist follows the same `validate` modes the [renderer](/docs/dev/blocks/rendering#validation) already uses, and it *skips*, never rejects:
