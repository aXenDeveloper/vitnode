...
The zone's allowlist and the field's `allowed` are usually the same value, and writing it twice is a typo waiting to happen. Put it in a module of its own and import it from both:
