...
[layout area](/docs/dev/blocks/layout-areas) counts towards the limit; the area
...
| `variant` | Optional. The look this instance picked, from the ones its block [declares](/docs/dev/blocks/block-variants). Absent means "the block's default". |

Array order **is** the rendered order. Nothing sorts it.

### Areas in the same list

A zone holds **content nodes**, and a node is either a block instance or an [Area](/docs/dev/blocks/layout-areas) - a small layout container with its own id, a `layout` of four tokens, and `children` that are blocks:
...
```json
{
  ...
  "children": [{ "id": "01JC…", "type": "core:text", "data": { "heading": "Left" } }]
}
```
...
A `blocks()` field is the storage half, and the [Visual Editor](/docs/dev/blocks/visual-editor) is how somebody arranges what goes in it. The two are joined by whatever your page hands the editor and whatever your API does with the payload that comes back - the field's own `allowed`, `min` and `max` are re-checked on the way in either way.

For a zone that is not a column on a record at all - a settings screen, a forum index, anything a page simply *has* - the storage half is core's, and you declare the zone rather than a field. See [Editable Pages](/docs/dev/blocks/editable-pages).
...
| **Not an AdminCP form field** | The generated form and the DataTable both leave it out - a zone is arranged on the page by the [Visual Editor](/docs/dev/blocks/visual-editor), not typed into a control. |
