...
Every create and update runs the zone through the registry, and the whole request is refused if any instance fails:
...
The column moves to the generated translation table, exactly as a localized `text` field does. Each language then has its own blocks and its own order, with the translation lifecycle, revisions and preview it already had - no new concepts.
