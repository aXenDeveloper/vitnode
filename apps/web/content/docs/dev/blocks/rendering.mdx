...
Evaluating `src/blocks.gen.ts` also installs that registry as the process default, so a renderer with no `registry` prop finds one. That fallback exists because two callers cannot be handed a registry: a renderer deep in a tree, and the Zod schema a content type builds while its plugin is still loading, long before an application exists. With neither, `ContentRenderer` throws a `BlockRegistryMissingError` naming both ways out.
