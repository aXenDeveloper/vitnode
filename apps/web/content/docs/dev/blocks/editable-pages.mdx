...
`EditablePage` also adopts the canonical answer a save comes back with - the stored zones **and** the layout metadata around them, `updatedAt` included. View mode shows the stored result the moment you press **Finish editing**, with no reload and no second request, and a "Last rearranged 2 minutes ago" line on the page moves with it instead of sitting on the timestamp the loader happened to read. Hand it a layout belonging to a different page id and it throws, because rendering one page's blocks under another page's zones is how a save writes them onto the wrong page.
