...
description: A variant is how one block instance looks, stored beside its data instead of inside it - declare a few with defineBlock and the editor grows a picker for free.
icon: Shapes
---

Sooner or later somebody asks for "the same callout, but smaller". You have three options: a second block type, a `size` field in the middle of the copy, or a **variant**.
...
```text
data     what the block says          "Ships on Friday"
variant  how this instance says it    "grid" | "list" | "compact"
layout   how blocks sit together      an Area's job, not a block's
```

Keep those three apart and every screen that touches a block stays honest. Mix them and a block definition slowly turns into a settings page.
...
Two optional keys on `defineBlock`:

```tsx title="plugins/example/src/blocks/features.tsx"
...
| Key | Rule |
| :--- | :--- |
| `variants` | A list of `{ id, label?, description? }`. Leave it off entirely if the block has one look - `variants: []` is refused, because a picker of nothing is worse than no picker. |
...
```json
{
  ...
  "variant": "list",
  "data": { "heading": "One block, three shapes", "primary": { "title": "Data" } }
}
```
...
```ts
import { resolveBlockVariant } from "@vitnode/core/blocks"

resolveBlockVariant(featuresBlock, undefined) // { kind: "resolved", variant: "grid" }
resolveBlockVariant(featuresBlock, "list")    // { kind: "resolved", variant: "list" }
resolveBlockVariant(featuresBlock, "carousel")// { kind: "unknown",  variant: "carousel" }
```
...
| Blocks side by side in columns | An [**Area**](/docs/dev/blocks/layout-areas). Arranging *several* blocks is never one block's job. |
