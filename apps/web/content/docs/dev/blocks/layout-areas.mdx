---
title: Layout Areas
description: An Area is a small layout container stored inside a zone - it holds blocks in up to four columns, it is not a block itself, and in Stage 4 it cannot hold another Area.
icon: Columns3
---

A zone is a list. Lists go downwards. Pages, unfortunately, also go sideways.

An **Area** is the smallest thing that fixes that: a container stored in the zone, holding a few blocks and one small layout description. It is what you reach for when two blocks belong next to each other, and nothing more than that.
...
| **[`variant`](/docs/dev/blocks/block-variants)** | One block instance | `grid`, `list`, `compact` |
...
```tsx
<ContentZone id="settings:sidebar" allowedBlocks={["core:text"]} blocks={content.sidebar} />
```
...
| **Filling it** | Drag a block onto the Area, or use **Add block** on the Area itself - the sidebar then says *For: an area in that zone*, and the Layout section disappears while it is targeted. |
...
| **Delete area** | Removes the Area **and** its children. A non-empty Area asks first, and offers *Ungroup, keep blocks* in the same breath. |

An **empty Area renders nothing at all in public** - no wrapper, no gap, exactly like an [empty zone](/docs/dev/blocks/content-zones#empty-zones). In edit mode it is a visible two-column drop target, because now somebody is looking for a place to put something.
...
```ts
import { contentNodeBlocks, isBlockAreaInstance } from "@vitnode/core/blocks"
...
| One block that looks different | A [**variant**](/docs/dev/blocks/block-variants). |
| A different arrangement per breakpoint | Neither, yet. One arrangement, rendered responsively. |
| A page-wide template - sidebar, footer, header | Several [**zones**](/docs/dev/blocks/content-zones) in your own page markup. An Area arranges content inside a zone; it is not a page layout. |
