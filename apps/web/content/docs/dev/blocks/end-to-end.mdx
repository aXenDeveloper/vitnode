...
[Content Zones](/docs/dev/blocks/content-zones) put a name on the place those blocks are rendered, so the step above becomes:

```tsx
<ContentZone id="main" blocks={page.content} registry={blocksRegistry} />
```

The Visual Editor then sits on top of exactly this. **Edit page** turns an editing surface on for an authorized editor; the block picker lists what the zone's allowlist resolves to; and drag and drop re-orders the array whose order you have already been storing.
