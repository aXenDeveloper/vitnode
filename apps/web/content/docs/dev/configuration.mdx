...
dependency pre-bundling and the plugin route generator in the order they have to
run:
...
only when something in the browser genuinely reads it. The three plugins are
still exported individually - `vitNodeEnv`, `vitNodeOptimizeDeps`,
`vitNodePluginRoutes` - if you ever need to drop or reorder one.
