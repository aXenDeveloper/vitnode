...
When fetching localized content through the public API or the delivery resolve
endpoint, translatable fields (`title`, `content`) resolve in the active locale.
...
export const ArticleCard = ({ article }: Props) => {
  ...
  return (
    <div className="card">
      <span className="badge">{t("content.article.badge_label")}</span> {/* [!code ++] */}
      <h3>{article.title}</h3>
      <p>{article.content}</p>
    </div>
  );
