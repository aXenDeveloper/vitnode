...
Content Engine automatically builds interactive management screens in the VitNode AdminCP without requiring you to write page components or custom form components.

<Callout title="Where these registrations live">
  This page is about *what* each override does. Which module you declare it in -
  `admin/nav`, `admin/content` or `config.tsx` - is
  [Plugin Frontend Modules](/docs/dev/content-engine/plugin-registration), and the
  short answer is `admin/content`.
</Callout>
...
<Callout type="warn" title="A path is an address people have bookmarked">
...
`ContentFormHeader` is the page heading - the title, the description and the
**Back to …** link - rendered inside the form so a layout can seat the submit row
beside the back link. It renders nothing in a dialog, so one layout serves both
surfaces. `ContentFormActions` holds the submit buttons: **Save changes** while
editing, **Create** while creating, and for a content type with `publication`,
**Save as draft** and **Publish** - the second only for a role holding
`can_publish`, and it runs the very same publish action the list's button does,
right after the create. While editing such a record, the same role also gets
**Publish** / **Convert to draft** beside **Save changes**: it moves the lifecycle
behind the usual confirmation and leaves whatever is typed in the form untouched.

<Callout type="warn" title="A page-mode layout places the header itself">
  Just as it places every field, a layout used on a create or edit *page* has to
  render `ContentFormHeader` - without it the page has no heading, no way back and,
  unless `ContentFormActions` sits somewhere else, no way to save. In development
  the engine logs a warning when a layout forgets it.
</Callout>
