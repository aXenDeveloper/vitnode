---
title: Cache
description: Learn how caching works in VitNode across the app runtime and Redis.
---
...
<Callout type="info" title="Which app cache you have depends on the runtime">
  On **TanStack Start** the app cache is TanStack Query - a route loader warms an
  entry, the component reads the same one back, and a mutation invalidates
  exactly what it changed. See [Data
  loading](/docs/dev/tanstack/data-loading).

On **Next.js** it is `"use cache"` and the tag API, which is what the rest of
this page describes. The Redis layer below is shared by both, because it lives
in the API.
