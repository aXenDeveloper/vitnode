...
description: Declare a page's zones once, and the people who run the site rearrange them on the page itself - Edit widgets from the user menu, drag, Save, done. No admin screen, no zone-to-column mapping, no hand-written save route.
icon: PanelsTopLeft
---
...
| `canEdit`     | optional | `true` mounts the editor runtime and offers **Edit widgets** in the user menu. `false` does neither. |
| `openEditing` | optional | Open in edit mode on arrival - what `?edit=true` asks for. Ignored unless `canEdit`.                 |
| `adapter`     | optional | Where **Save** goes. `createContentEditorAdapter` builds one.                                        |
| `onExit`      | optional | Called by **Finish editing**, after the unsaved-changes prompt. Edit mode ends either way.           |
...
```tsx title="Any page that declares its zones"
...
<Callout title="When to reach past it">
  Write the loader and adapter by hand when the page is not read from core's own
  layout route - zones stored on your own record, a different API, a preview of
  something unsaved. The sections above are that contract, and `<EditablePage>`
  is still what you wrap them in.
</Callout>
...
| Zone     | Where it sits                                       |
| :------- | :-------------------------------------------------- |
| `header` | Under the "Settings" heading, above the two panels. |
| `footer` | Below the panels, above the site footer.            |
...
Rearranging them is gated on the moderator permission `@vitnode/core` → **Widgets** → `can_edit`, grantable from **Staff → Moderators** in the AdminCP. Hold it (or a root role) and **Edit widgets** appears in the user menu while you are on `/settings`, exactly as it does on a page your own plugin declared.
