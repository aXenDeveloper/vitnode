...
<Callout type="info" title="Upgrading an existing index">
  Documents now carry `authorIds` (a list) instead of `authorId`, plus drafts
  as private documents. If your index was created before that, delete it in
  Elasticsearch and run **Rebuild index**. VitNode recreates it with the new
  mapping.
</Callout>
