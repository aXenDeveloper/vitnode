...
<Callout type="info" title="Using the Content Engine?">
  ...
  block](/docs/dev/content-engine/public-api-and-caching#step-3-enable-global-search-indexing)
...
an **API-process** model: only something running inside the API holds that
context, so a route loader, a component or a server function reaches search the
same way it reaches everything else - over HTTP.
...
The search UI ships in three framework-neutral pieces, so an app route and a
plugin route render the same components:
